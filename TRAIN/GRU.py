...
from tensorflow.keras.layers import Embedding, GRU, Dense, Dropout, Activation, Flatten, Masking
...
def train_gru(n_symbols, embedding_weights, x_train, y_train, x_validate, y_validate):
...
def train():
    ...
    for sentence in x_test:
        words = sentence.split()
        data.append(words)
    index_dict, word_vectors, x_test = create_dictionaries(model=Word2Vec.load('./Word2vec_model.pkl'), combined=data)

    with open('./traindata_x2.txt', 'w') as f:
        for i in x_test:
            f.write(i)

    print('Huấn luyện với GRU...')
    train_gru(n_symbols, embedding_weights, x_train, y_train, x_validate, y_validate)
